...
int main(){
    ...
    double **A3 = three_stencil(l3); // coarsest grid
    ...
    double *r3 = new double[l3]; // coarsest grid
    ...
    double *e3 = new double[l3]; // coarsest grid
    initialize_vec(e3, l3);
    ...
    //for(int i = 0; i < 2; i++){
        ...
        r2_new = getResidual(A2, r2, e2, l2);

        // second restriction
        r3 = restrict(r2_new, l2);

        // solve the error equation Ae=r using GS solver
        initialize_vec(e3, l3);
        e3 = GS(l3, 1, A3, r3, e3);
        double *temp = getResidual(A3, r3, e3, l3);
        double error = norm_max(temp, l3);
        double conv = 1e-7;
        while (error > conv){
            e3 = GS(l3, 1, A3, r3, e3);
            temp = getResidual(A3, r3, e3, l3);
            error = norm_max(temp, l3);
        }

        // first prolongation
        e2_new = prolong(e3, l3);

        // correct the solution e2 for the coarse grid
        e2 = add_vv(e2, e2_new, l2);

        // post-relaxation/smoothing
        e2 = GS(l2, 1, A2, r2, e2);
